...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.no_input==3:
            self.input_A3 = self.Tensor(nb, opt.input_nc, size, size) # store inputs in a tensor # DONE
        ...
        if self.no_input==2:
            ...
            self.netG_A = (networks.define_G(opt.input_nc, opt.output_nc,
                                            opt.ngf, 'resnetMM_3', opt.norm, not opt.no_dropout, 						opt.init_type, self.gpu_ids))
            #inputs = torch.randn(1,3,256,256)
            #y = self.netG_A(Variable(inputs).cuda(),Variable(inputs).cuda())
            #g = make_dot(y)
            #g.view()
            self.netG_B=(networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, 'resnetMMReverse_3', opt.norm, not opt.no_dropout, 						opt.init_type, self.gpu_ids))
    ...
    def set_input(self, input):
        ...
        self.image_paths = input['A1_paths' if AtoB else 'B_paths']
    ...
    def backward_D_B(self):
        ...
        if self.no_input==2:
            ...
            self.loss_D_B =0.5*( self.backward_D_basic(self.netD_B1, self.real_A1, fake_A1)+self.backward_D_basic(self.netD_B2, self.real_A2, fake_A2)++self.backward_D_basic(self.netD_B3, self.real_A3, fake_A3))
    ...
    def backward_G(self):
        ...
        if self.no_input==2:
            ...
            self.loss_cycle_A = (self.criterionCycle(self.rec_A1, self.real_A1) * lambda_A+self.criterionCycle(self.rec_A2, self.real_A2) * lambda_A+self.criterionCycle(self.rec_A3, self.real_A3)* lambda_A)
